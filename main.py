...
def add_domain(domain, guildid):
    ...
    if domain not in prevdomains:
        ...
        c.execute("UPDATE guilds SET domains=? WHERE guildid=?",
...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Game(name='.vstatus | Verify Yourself'))
        ...
        if user_prev_verify == None:
            ...
            if role:
                ...
                role = discord.utils.get(
                    member.guild.roles, name=check_on_join[3])
                await member.add_roles(role)
  ...
  if (message.guild == None) and email_check(message_content):
    ...
    if len(users_guilds) >= 1:
      ...
      if len(verif_list) >= 1:
        ...
        emailmessage = Mail(
            ...
            subject='Verify Your SAKEC email for CSI-SAKEC Coding Community',
        ...
        try:
          ...
          print("is", type(mailgun_email), "and is equal to ", mailgun_email)
    ...
    if len(prev_codes) >= 1:
      for i in prev_codes:
        ...
        unverifed = discord.utils.get(curr_guild.roles, name='Unverified')
        member = curr_guild.get_member(message.author.id)
        if unverifed in member.roles:
          useremail = i[2]
          username = useremail.split("@")[0]
          await member.edit(nick=username)
          await member.remove_roles(unverifed)
          if role:
            if role not in member.roles:
              await member.add_roles(role)
            else:
          ...
          await message.channel.send("You don't have **Unverified** role. Please contact the Admin.")
    else:
...
async def vstatus(ctx):
    if ctx.guild:
        ...
        await ctx.send("```" +
                       ...
                       "   .addasmember [emails] ->Gives user CSI-Member role" + "\n" +
                       "   .removeasmember [emails] -> Removes user CSI-Member role" + "\n" +
                       "   .updatemembership [emails] -> Only this users will have the CSI-Member role" + "\n" +
...
@client.command()
async def removeasmember(ctx, *emails):
    if ctx.guild and ctx.author.guild_permissions.administrator:
      check_on_join = get_guild(ctx.guild.id)
      if check_on_join == None:
          new_guild(ctx.guild.id)
      check_on_join = get_guild(ctx.guild.id)
      csi_member_role = discord.utils.get(ctx.guild.roles, name='CSI-Member')
      for email in emails:
        userid = email.split("@")[0]
        user = ctx.guild.get_member_named(userid)
        if (not user):
          await ctx.send(f"SKIP: User {userid} was not found. ")
        else:
          await user.remove_roles(csi_member_role)
          await ctx.send(f"CSI-Member role removed from user: ({userid})")


@client.command()
async def addasmember(ctx, *emails):
    if ctx.guild and ctx.author.guild_permissions.administrator:
      check_on_join = get_guild(ctx.guild.id)
      if check_on_join == None:
          new_guild(ctx.guild.id)
      ...
      for email in emails:
        ...
        if (not user):
          ...
          await user.add_roles(csi_member_role)
          await ctx.send(f"CSI-Member role added to user: ({userid})")


async def updatemembership(ctx, *emails):
    if ctx.guild and ctx.author.guild_permissions.administrator:
      check_on_join = get_guild(ctx.guild.id)
      if check_on_join == None:
          new_guild(ctx.guild.id)
      ...
      userids = []
      for email in emails:
        userids.append(email.split("@")[0])

      for member in ctx.guild.members:
        if member.id in userids:
          if not (csi_member_role in member.roles()):
            await member.add_roles(csi_member_role)
        else:
          if (csi_member_role in member.roles()):
            await member.remove_roles(csi_member_role)
